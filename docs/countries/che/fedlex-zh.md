...
| 稳定性 | **查询端点存在降级窗口**（2026-09-13 实测）：不稳定时段内，复杂查询（多块 OPTIONAL）会收到**合法形状的空结果**（不是报错），同一查询时好时空、无规律可循；简单查询不受影响。框架对策：条目/谱系类任务把零行视为瞬时错误自动重试（重试耗尽升级人工，绝不把空结果当数据入账）；采集后对源站做独立计数复核可发现漏抓 |
...
每种任务 = 一次请求 + 一次解析。共 **6 种**，分属两层（参数入口也分层）：
...
| `che_sr_entry` | SPARQL：单条目元数据（SR 分类号、basicAct、生效窗口/状态、类型、各语言标题） | `sr_entries` 一行（upsert，锚点指针暂空）；携带各语言标题生成 `che_sr_versions` |
| `che_sr_versions` | SPARQL：单条目版本谱系 + 锚点版文件直链（条目与其谱系**拆成两个任务**：查询端点的降级窗口会清空大连接查询，而两半各自稳定——2026-09-13 实测） | `sr_versions` 整组重写；`sr_entries` 锚点指针补写；锚点版每语言一个 `che_file` |
| `che_file` | GET 门户域文件库（图内直链） | 魔数校验（HTML/PDF 按格式）→ 文件落盘 + documents 一行（多语言口径见 §4） |

任务链——AS：`che_as_day(日) → che_file × (3 语 × 1 格式)`；SR 回填：`che_sr_walk → che_sr_entry → che_sr_versions → che_file`；SR 增量：`che_sr_feed → che_sr_entry → che_sr_versions → che_file`。
...
**账本内计数**（2026-09-13 首次实跑）：窗口 2024-01-03..05 = 18 任务零失败（3 日任务 + 15 文件任务）、`as_works` 5 行、documents 15 行（5 作品 × de/fr/it）、磁盘文件与账本哈希逐一吻合、同窗重跑零请求（幂等）；与旧系统基线（同窗 5 文档）按作品多重集逐一相等。
...
| **猜测文件地址的陷阱** | 不存在的 filestore 地址返回 HTTP 206 + HTML 错误页（不是 404）——一切下载地址以图内 `isExemplifiedBy` 为准 + 响应魔数校验，绝不自行拼 URL。**魔数校验容忍前导空白**：真实 html 文件以一个换行符开头（2026-09-13 实测，首字节 `
<!DOCTYPE`） |
...
*更新日期：2026-09-13；数据快照：2026-09-09（源站实测）；数据由 fedlex 源 as_window=2024-01-03:2024-01-05 首次实跑背书（18 任务零失败 / 15 文档三方对账一致 / 幂等重跑零请求；SR 侧走查随端点降级窗口恢复后补记）。*
